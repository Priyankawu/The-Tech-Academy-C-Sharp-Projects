...
{
    public class TwentyOneGame : Game, IWalkAway         //inherits from Game class
    {
        ...
        public override void Play()
        {
            ...
            foreach (Player player in Players)
            {
                ...
                if (!successfullyBet)
                {
                    return;                         //void method - this return just means end this method
                }
                Bets[player] = bet;
            }
            for (int i=0; i<2; i++)
            ...
            {
                Console.WriteLine("Dealr is hitting... ");
                Dealer.Deal(Dealer.Hand);
                Dealer.isBusted = TwentyOneRules.IsBusted(Dealer.Hand);
                Dealer.Stay = TwentyOneRules.ShouldDealerStay(Dealer.Hand);
            }
            ...
            if (Dealer.isBusted)
            {
                ...
                foreach (KeyValuePair<Player, int> entry in Bets)
                {
                    ...
                    Players.Where(x => x.Name == entry.Key.Name).First().Balance += (entry.Value * 2);  //Where produces a list
                    Dealer.Balance -= entry.Value;
                }
                return;
            }
